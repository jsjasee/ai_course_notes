...
class Item(BaseModel):
    ...
    def make_prompts(self, tokenizer, max_tokens, do_round):
        ...
        self.prompt = f"{QUESTION}\n\n{summary}\n\n{PREFIX}"  # we put the prefix to force the model to give us its prediction.
        self.completion = (
            f"{round(self.price)}.00" if do_round else str(self.price)
        )  # train the model to predict the price to the nearest cents, do_round is true when we are using the model to generate the price. if we are building the test dataset, use the raw price, don't round.
